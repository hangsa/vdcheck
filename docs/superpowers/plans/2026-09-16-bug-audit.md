...
**Python requirement:** `video_checker.py` uses PEP 604 union syntax (`dict | None`) without `from __future__ import annotations`, so it requires Python 3.10+. On macOS/Linux, system `python3` is often 3.9. All `python3` commands in this plan assume a 3.10+ interpreter. If `python3 --version` reports <3.10, substitute a 3.10+ interpreter explicitly (e.g., `python3.11`, `/Users/longsa/.local/bin/python3.11`, or an activated pyenv/venv).
